...
\newcommand{\draft}[1]{
  ...
  % use dashes as separators for dates (deffault in git, so it is needed
  % for consistency)
  ...
  % define \gitBranchSymbol and \gitIfIsDirtyRepoSymbol

  % intermediate commands to define \gitBranchSymbol: start from
  % \hookrightarrow, rotate it 45 degrees and scale it by 1.2 times
  \newcommand{\rothookrightarrow}{\rotatebox[origin=c]{45}{$\hookrightarrow$}}
